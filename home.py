...
if st.button("กดตรงนี้เพื่อดูผล"):
   ...
   if out[0] == 1:
    ...
    st.markdown("<h2 style='text-align: center; color: blue;'>เสียใจด้วย คุณเป็นโรคเบาหวาน</h2>",unsafe_allow_html=True)
   elif out[0] == 0:
    st.image('./img/TWO.jpg', width=1000)
    st.markdown("<h2 style='text-align: center; color: blue;'>ยินดีด้วย คุณไม่เป็นโรคเบาหวาน</h2>",unsafe_allow_html=True)
